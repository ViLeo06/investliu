...
def main():
    ...
    OUTPUT_FILE = "/mnt/c/Users/M2814/.cursor/investliu/laoliu_notes_ocr_batch1.txt"
    ...
    print(f"开始处理前 {len(test_files)} 个文件...")
    print("=" * 50)
    
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        f.write("# 老刘投资笔记 - OCR文字提取结果 (批次1)\n")
        f.write(f"# 处理时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"# 处理文件数: {len(test_files)}\n\n")
    ...
    print("=" * 50)
    print(f"✓ 批次1处理完成！结果已保存到: {OUTPUT_FILE}")
